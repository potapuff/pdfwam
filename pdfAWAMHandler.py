...
class PdfTblStruct(object):
    ...
    def add(self, elem):
        ...
        typ = elem['/S']
        # Not a table element ?
        if typ not in self.typedict.keys():
            return 0
...
class PdfAWAMHandler:
    ...
    """
    def __init__(self, resultMap=None,roleMap=None,validateImages=False,ignoreSingleBitImgs=False,debug=False):
        if resultMap==None:
            self.resultMap={}
        else:
    ...
    def getFlattenedPages(self, pgelem):
        ...
        parent = pgelem['/Parent'].getObject()
        pages = []
        ...
        pages = [pg.getObject() for pg in pages if pg != None]
        return pages
    ...
    def awamHandler(self,element):
        ...
        if structureType=='/Link':
        ...
        elif structureType in PdfTblStruct.typedict.keys():
            ...
            except PdfTblStructInvalidException, e:
                pass
            ...
            try:
                ...
                if self.validateImgs:
                    ...
                    try:
                        ...
                        for page in pages:
                            pgobj = page.getObject()
                            if pgobj == pg:
                                # This is the page, validate the page
                                res = pg['/Resources'].getObject()
                                try:
                                    xobj = res['/XObject'].getObject()
                                    imgs=[item.getObject() for item in xobj.values() if \
                                          item.getObject().get('/Subtype') in ('/Image')]
                                    ...
                                    if any(imgs):
                                        ...
                                        if self.ignore1bitimgs:
                                            try:
                                                bits = [(img.getObject()['/BitsPerComponent'] != 1) for img in imgs]
...
if __name__ == '__main__':
    ...
    pdfElement={'/Lang': u'no-NO', '/S': '/Document'}
