...
__version__ = "2.0"
__lastmodified__ = "Mon Aug 13 01:29:45 IST 2012"
...
class PdfWCAG(pdfStructureMixin.PdfStructureMixin):
        ...
        if test_id in self.test_ids:
            ...
            try:
                ...
                if (type(ret) is int) and ret != 2:
                    ...
                    for status, pagedict in ret.iteritems():
                        for page, items in pagedict.iteritems():
                            for item in items:
            ...
            try:
                ...
                for key in new_keys:
                    ...
                    if (type(val) is int) and val != 2:
                        ...
                        for (page, count), status in val.iteritems():
                            if status: succ += 1
                            else: fail += 1
    ...
    def test_WCAG_PDF_11_13(self, wamdict):
        ...
        linkObjs = [x[0] for x in self.awamHandler.linkAnnots.values()]
    ...
    def test_WCAG_PDF_14(self):
        ...
        for pg in range(self.numPages):
            ...
            # print 'Art elems =>',pg,len(artElems)
            for artifactElems in artElems:
                # First element is the artifact element
                artifact, artype = artifactElems[0]
                # print artifact, artype
                # print 'CONTENT:',self.getArtifactContent(artifactElems)
                
                # Skip this
                if (len(artifact) < 2): continue
                ...
                try:
                    ...
                    if (atype == '/Pagination'):
                        if artifactDict.has_key('/Subtype'):
        ...
        if (self.numPages == 1) and (len(pgKeys) == 0):
            # Not applicable
            return 2
        ...
        for pgnum in range(1, self.numPages):
            pgid = str(pgnum+1)
